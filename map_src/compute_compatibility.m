...
compatibility.ic = false(size(observations.m, prediction.n));
compatibility.candidates.features =  zeros(1, prediction.n);
compatibility.candidates.observations =  zeros(1, observations.m);%zeros(observations.m, 1);
compatibility.AL = zeros(1, observations.m);%zeros(observations.m, 1);
compatibility.HS = 0;
