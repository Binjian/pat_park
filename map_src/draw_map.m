...
function draw_map (map, ground, configuration, step)
%-------------------------------------------------------
...
%axis([-2 12 -2 12]);
% draw vehicle
draw_vehicle(map.x(1:3), map.P(1:3,1:3), 'b', configuration);

% ground truth for trajectory
draw_trajectory(ground.trajectory, 'r', configuration, step);

% trajectory up to now
draw_trajectory(map.estimated, 'b',configuration);

% odometry up to now
draw_trajectory(map.odometry, 'g',configuration);
